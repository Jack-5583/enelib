...
import { inclassFindArticleId, inclassFetchAnswer, inclassViewUrl } from "@/lib/inclass";
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!articleId) articleId = await inclassFindArticleId(q.subject);
    if (!articleId) return NextResponse.json({ answerText: q.answerText, answeredAt: q.answeredAt });

    const answer = await inclassFetchAnswer(articleId);
    const updated = await prisma.inclassQuestion.update({
      where: { id: q.id },
      data: {
        articleId,
        articleUrl: inclassViewUrl(articleId),
